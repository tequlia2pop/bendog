...
public final class RequestUtil {
	...
	/**
	 * 根据 RFC 2109 将 cookie 请求头解析为 cookie 数组。
	 * 
	 * @param header HTTP "Cookie" 请求头的值
	 */
	...
	/**
	 * 将指定 String 中的请求参数添加到指定的 Map。假定不会从任何其他的线程访问指定的 Map，因此不执行同步。
	 * <p>
	 * <strong>IMPLEMENTATION NOTE</strong>: 对解析到的名称和值元素单独执行 URL 解码
	 * （而不是提前在整个查询字符串上执行 URL 解码），
	 * 对名称或值包含编码的 "=" 或 "&" 字符的情况进行适当的处理，否则将其解释为分隔符。
	 *
	 ...
	 */
	public static <K,V> void parseParameters(Map<String, String[]> map, String data, String encoding)
			throws UnsupportedEncodingException {
		if ((data != null) && (data.length() > 0)) {
			/*int len = data.length();
			byte[] bytes = new byte[len];
			data.getBytes(0, len, bytes, 0);*/
			byte[] bytes = data.getBytes();// lost encoding
	...
	public static void parseParameters(Map<String, String[]> map, byte[] data, String encoding)
		...
		if (data != null && data.length > 0) {
			int ix = 0;// data[] 的当前位置
			int ox = 0;// 每次解析得到的名/值保存在 data[] 的头部，ox 表示它们的当前位置
			String key = null;
			String value = null;
